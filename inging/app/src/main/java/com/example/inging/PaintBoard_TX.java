...
public class PaintBoard_TX extends View {
	...
	public static void touch_start(int x, int y){
		...
		//Launcher.cycle = 0;
		MainActivity.mode(x, y, x, y, "0", input_color, stroke, MainActivity.PenStatus, MainActivity.pcount);
		MainActivity.pcount++;
		mPath.moveTo(x, y);
		mX = x;
		mY = y;
	...
	public static void touch_move(int x, int y){
		...
		if(mMode == 0 || mMode == 1){
			MainActivity.mode(mX, mY, x, y,"1", input_color, stroke, MainActivity.PenStatus, MainActivity.pcount);
	...
	public static void touch_up(){
		...
		if(preX == mX && preY ==mY){
			MainActivity.mode(mX, mY, mX, mY, "3", input_color, stroke, MainActivity.PenStatus, MainActivity.pcount);
			if(mCanvas != null ){
				mCanvas.drawPoint(preX, preY, mPaint);
			}
		}
		else{
			MainActivity.mode(mX, mY, mX, mY, "2", input_color, stroke, MainActivity.PenStatus, MainActivity.pcount);
			mPath.lineTo(mX, mY);
			if(mCanvas != null)
				mCanvas.drawPath(mPath, mPaint);
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
            ...
            case MainActivity.RECT:
            	switch(MainActivity.PenStatus){
            	case MainActivity.mode0:
            		switch(MainActivity.PenStyle){
            		case MainActivity.nomal:
            			mPaint.setStyle(Paint.Style.STROKE);
            			touch_start(x, y);
                		mPaint.setMaskFilter(filter0);
